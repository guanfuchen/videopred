...
from .config import moving_mnist_config
...
def load_dataset():
    ...
    return load_mnist_images(moving_mnist_config.moving_mnist_data)


# generates and returns video frames seqs*seq_len*(shape)*1
def generate_one_channel_moving_mnist_batch(shape=(64,64), seq_len=10, seqs=4, num_sz=28, nums_per_image=2):
    mnist = load_dataset()
    width, height = shape
    lims = (x_lim, y_lim) = width-num_sz, height-num_sz
    dataset_batch = np.empty((seqs, seq_len, height, width, 1), dtype=np.uint8)
    sample = np.empty((seq_len, height, width, 1), dtype=np.uint8)
    minpos = 100
    maxpos = 0
    for seq_idx in xrange(seqs):
        ...
        for frame_idx in xrange(seq_len):
            ...
            sample_mnist = (canvas * 255).astype(np.uint8).clip(0,255).transpose((2, 1, 0))
            # sample_mnist = sample_mnist[:, :, 0]
            # sample_mnist = np.array((sample_mnist,)).transpose((1, 2, 0))
            # print(sample_mnist.shape)
            sample[frame_idx] = sample_mnist
        #print(sample.shape)
        dataset_batch[seq_idx] = sample
    return dataset_batch

# generates and returns video frames seqs*seq_len*(shape)*3
def generate_moving_mnist_batch(shape=(64,64), seq_len=10, seqs=4, num_sz=28, nums_per_image=2):
    mnist = load_dataset()
    width, height = shape
    lims = (x_lim, y_lim) = width-num_sz, height-num_sz
    dataset_batch = np.empty((seqs, seq_len, height, width, 3), dtype=np.uint8)
    sample = np.empty((seq_len, height, width, 3), dtype=np.uint8)
    minpos = 100
    maxpos = 0
    for seq_idx in xrange(seqs):
        # if seq_idx % 1000 == 0:
        ...
        for frame_idx in xrange(seq_len):
            ...
            sample_mnist = (canvas * 255).astype(np.uint8).clip(0,255).transpose((2, 1, 0))
            sample_mnist = sample_mnist[:, :, 0]
            sample_mnist = np.array((sample_mnist,)*3).transpose((1, 2, 0))
            # print(sample_mnist.shape)
            sample[frame_idx] = sample_mnist
        #print(sample.shape)
        dataset_batch[seq_idx] = sample
...
def generate_moving_mnist(shape=(64,64), seq_len=10, seqs=30, num_sz=28, nums_per_image=2):
    mnist = load_dataset()
    width, height = shape
    lims = (x_lim, y_lim) = width-num_sz, height-num_sz
...
def main():
    ...
    dataset_batch = generate_moving_mnist_batch()
    print(dataset_batch.shape)
    # np.savez('dataset_batch.npz', dataset_batch)
    
    one_channel_dataset_batch = generate_one_channel_moving_mnist_batch()
    print(one_channel_dataset_batch.shape)

...
from videopred.dataloader import moving_mnist

FLAGS = tf.app.flags.FLAGS
tf.app.flags.DEFINE_integer('batch_size', 4, """the training batch size""")
...
def train():
    with tf.Graph().as_default():
        ...
        for step in range(FLAGS.max_step):
            if channel == 3:
                dataset_batch = moving_mnist.generate_moving_mnist_batch(shape=(height, width), seq_len=time,seqs=FLAGS.batch_size)
            elif channel == 1:
                dataset_batch = moving_mnist.generate_one_channel_moving_mnist_batch(shape=(height, width), seq_len=time,seqs=FLAGS.batch_size)

...
from moving_mnist import generate_moving_mnist_batch
...
def basic_lstm_network(inputs, hidden):
    ...
    outputs = transpose_conv_layer(conv7, kernel_size=3, stride=2, num_features=3, idx='decode_8')
...
def train():
    with tf.Graph().as_default():
        ...
        for step in range(FLAGS.max_step):
            dataset_batch = generate_moving_mnist_batch(shape=(height, width), seq_len=time,seqs=FLAGS.batch_size)
